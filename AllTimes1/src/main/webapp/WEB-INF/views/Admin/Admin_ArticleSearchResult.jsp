			...
			<tbody>
				<c:forEach items="${searchResult }" var="search">
					<tr>
						...
						<c:choose>
							<c:when test="${search.ar_genre == 'POLITICS'}">
								<td>정치</td>
							</c:when>
							<c:when test="${search.ar_genre == 'ECONOMY'}">
								<td>경제</td>
							</c:when>
							<c:when test="${search.ar_genre == 'SOCIETY'}">
								<td>사회</td>
							</c:when>
							<c:when test="${search.ar_genre == 'CULTURE'}">
								<td>문화</td>
							</c:when>
							<c:when test="${search.ar_genre == 'SPORT'}">
								<td>스포츠</td>
							</c:when>
							<c:when test="${search.ar_genre == 'INTERNATIONAL'}">
								<td>국제</td>
							</c:when>
							<c:when test="${search.ar_genre == 'SCIENCE'}">
								<td>IT/과학</td>
							</c:when>
							<c:when test="${search.ar_genre == 'COVID'}">
								<td>COVID-19</td>
							</c:when>
						</c:choose>
						<td>${search.ar_name }</td>
						<td>${search.ar_date }</td>
						<td>
							<a class="btn btn-sm btn-primary text-xs" href="Admin_ArticleInfoPage?ar_no=${search.ar_no }"><i class="check">상세 정보</i></a>
							<a class="btn btn-sm btn-danger text-xs" href="Admin_DeleteArticle?ar_no=${search.ar_no }">기사 삭제</a>
						</td>
